...
!--------------------------------------------------------------------------
  ...
  integer(IN) :: kz,ku,kv,kws,ktbot,kptem,kshum,kdens,kpbot,kpslv,klwdn
  ...
  integer(IN),parameter :: ktrans = 57
...
!-------------------------------------------------------------------------------
 ...
 if (phase == 1) then
   ...
   &                          scmlat=scmlat, scmlon=scmLon)
    ...
    kws   = mct_aVect_indexRA(a2x,'Sa_ws')
...
!EOP
   ...
   !--- temporaries
   real(R8)      :: uprime,vprime,utmp,vtmp,swndr,swndf,swvdr,swvdf,ratio_rvrf
...
!         a2x%rAttr(kptem,n) = a2x%rAttr(ktbot,n) / ((a2x%rAttr(kpbot)/1.0e6)**0.2854)
   ...
   case('CORE2_NYF','CORE2_IAF')
      ...
      do n = 1,lsize
         ...
         !--- (not needed for 20CRv2 winds!!)
         ! pure NCEP region:
         if ( abs(yc(n)) > 30.0_R8 ) then
            ...
            a2x%rAttr(kws,n) = sqrt(a2x%rAttr(ku,n)**2 + a2x%rAttr(kv,n)**2)
         endif
         ! NCEP/20CRv2 blend region:
         if ( abs(yc(n)) >= 20.0_R8 .and. abs(yc(n)) < 30.0_R8 ) then
            factor = 0.1_R8*(abs(yc(n))-20.0_R8) 
            uprime    = a2x%rAttr(ku,n)*windFactor(n)
            vprime    = a2x%rAttr(kv,n)*windFactor(n)
            utmp = uprime*cos(winddFactor(n)*degtorad)- &
                           vprime*sin(winddFactor(n)*degtorad)
            vtmp = uprime*sin(winddFactor(n)*degtorad)+ &
                           vprime*cos(winddFactor(n)*degtorad)
            a2x%rAttr(ku,n) = factor*utmp + (1.-factor)*a2x%rAttr(ku,n)
            a2x%rAttr(kv,n) = factor*vtmp + (1.-factor)*a2x%rAttr(kv,n)
            a2x%rAttr(kws,n) = factor*sqrt(utmp**2 + vtmp**2) + &
                           (1.-factor)*a2x%rAttr(kws,n)
         endif

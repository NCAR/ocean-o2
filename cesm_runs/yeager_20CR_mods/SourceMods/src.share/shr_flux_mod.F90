...
SUBROUTINE shr_flux_atmOcn(nMax  ,zbot  ,ubot  ,vbot  ,wsbot,thbot ,   & 
...
! !INPUT/OUTPUT PARAMETERS:
   ...
   real(R8)   ,intent(in) :: wsbot (nMax) ! atm wind speed       (m/s)
...
!       vmag   = max(umin, sqrt( (ubot(n)-us(n))**2 + (vbot(n)-vs(n))**2) )
        vmag   = wsbot(n)/max(umin,sqrt( ubot(n)**2 + vbot(n)**2) )
        vmag   = max(umin, vmag*sqrt( (ubot(n)-us(n))**2 + (vbot(n)-vs(n))**2) )
